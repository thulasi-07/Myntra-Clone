...
const Products = (props:productProp) => {
    ...
    return (
       ...
       {location.state?.category && <div className="flex items-center text-gray-500 text-sm ml-5 mt-2">
        ...
        {location.state?.category ? <div className="grid grid-cols-4 p-3 pl-5">
            {props.product.filter((data:any)=> data.category.name.includes(location.state.category)).map((data:any)=>{
                return <>
                <Link to="/details" state={{data:data}}></Link>
                <div>
